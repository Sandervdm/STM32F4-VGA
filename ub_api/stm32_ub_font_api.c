...
const static unsigned char FONT_STD[95][5] = {
	...
	while(*str != 0)
	{
		...
		for(j=0;j<CHAR_WIDTH;j++) {
			for(i=0;i<CHAR_HEIGHT;i++) {
				if(FONT_STD[Char_Pos][i] & (0b10000000>>j)) {
					UB_VGA_SetPixel(posY + OffsetCounter + i, posX + j, color);
				}
			}
		}

		//increment counters
		OffsetCounter += CHAR_OFFSET;
		str++;
	}
}
